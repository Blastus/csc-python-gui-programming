...
class Key(object):
    ...
    def __init__(self, **kwargs):
        ...
        if isinstance(key, (int, winreg.HKEYType)) and sub_key is None and sam is None and computer_name is None:
            self.__self = winreg.OpenKey(key, '')
        elif isinstance(key, Key) and sub_key is None and sam is None and computer_name is None:
            self.__self = winreg.OpenKey(key.__self, '')
        elif isinstance(key, (int, winreg.HKEYType)) and isinstance(sub_key, str) and sam is None and computer_name is None:
            self.__self = winreg.OpenKey(key, sub_key)
        elif isinstance(key, Key) and isinstance(sub_key, str) and sam is None and computer_name is None:
            self.__self = winreg.OpenKey(key.__self, sub_key)
        elif isinstance(key, (int, winreg.HKEYType)) and sub_key is None and isinstance(sam, int) and computer_name is None:
            self.__self = winreg.OpenKey(key, '', 0, sam)
        elif isinstance(key, Key) and sub_key is None and isinstance(sam, int) and computer_name is None:
            self.__self = winreg.OpenKey(key.__self, '', 0, sam)
        elif isinstance(key, (int, winreg.HKEYType)) and isinstance(sub_key, str) and isinstance(sam, int) and computer_name is None:
            self.__self = winreg.OpenKey(key, sub_key, 0, sam)
        elif isinstance(key, Key) and isinstance(sub_key, str) and isinstance(sam, int) and computer_name is None:
            self.__self = winreg.OpenKey(key.__self, sub_key, 0, sam)
        elif isinstance(key, int) and sub_key is None and sam is None and isinstance(computer_name, str):
            self.__self = winreg.ConnectRegistry(computer_name, key)
        elif isinstance(key, int) and isinstance(sub_key, str) and sam is None and isinstance(computer_name, str):
            self.__self = winreg.OpenKey(winreg.ConnectRegistry(computer_name, key), sub_key)
        elif isinstance(key, int) and sub_key is None and isinstance(sam, int) and isinstance(computer_name, str):
            self.__self = winreg.OpenKey(winreg.ConnectRegistry(computer_name, key), '', 0, sam)
        elif isinstance(key, int) and isinstance(sub_key, str) and isinstance(sam, int) and isinstance(computer_name, str):
            self.__self = winreg.OpenKey(winreg.ConnectRegistry(computer_name, key), sub_key, 0, sam)
        else:
    ...
    def save(self, file_name):
        ...
        winreg.SaveKey(self.__self, file_name)

    def load(self, sub_key, file_name):
        'Load subkey from file.'
        winreg.LoadKey(self.__self, sub_key, file_name)
    ...
    def __set_keys(self, keys):
        ...
        if isinstance(keys, str):
            winreg.CreateKey(self.__self, keys)
    ...
    def __del_keys(self):
        ...
        try:
            while True:
                winreg.DeleteKey(self.__self, winreg.EnumKey(self.__self, 0))
    ...
    def __set_values(self, values):
        ...
        if isinstance(values, str):
            winreg.SetValueEx(self.__self, values, 0, REG.SZ, winreg.QueryValue(self.__self, ''))
    ...
    def __del_values(self):
        ...
        try:
            while True:
                winreg.DeleteValue(self.__self, winreg.EnumValue(self.__self, 0)[0])
        except EnvironmentError:
            pass

    def __get_value(self):
        'Private class method.'
        return winreg.QueryValue(self.__self, '')

    def __set_value(self, value):
        'Private class method.'
        winreg.SetValue(self.__self, '', REG.SZ, value)

    def __del_value(self):
        'Private class method.'
        winreg.DeleteValue(self.__self, '')

    def __get_info(self):
        'Private class method.'
        return Info(*winreg.QueryInfoKey(self.__self))
...
class Keys(object):
    ...
    def __len__(self):
        ...
        return winreg.QueryInfoKey(self.__self)[0]
    ...
    def __setitem__(self, key, value):
        ...
        try:
            ...
            winreg.LoadKey(self.__self, key, file_name)
        finally:
            os.remove(file_name)

    def __delitem__(self, key):
        'Delete the specified key.'
        winreg.DeleteKey(self.__self, key)
    ...
    def __iter__(self):
        ...
        try:
            while True:
                keys.append(winreg.EnumKey(self.__self, index))
                index += 1
        except EnvironmentError:
            for key in keys:
                yield key
    ...
    def __contains__(self, item):
        ...
        try:
            while True:
                if winreg.EnumKey(self.__self, index).lower() == item:
                    return True
                index += 1
        except EnvironmentError:
            return False
...
class Values(object):
    ...
    def __len__(self):
        ...
        return winreg.QueryInfoKey(self.__self)[1]

    def __getitem__(self, key):
        'Return the specified value.'
        item_value, item_type = winreg.QueryValueEx(self.__self, key)
        return self.TYPES[item_type](item_value)

    def __setitem__(self, key, value):
        'Assign the item to a value.'
        if isinstance(value, self.TYPES):
            winreg.SetValueEx(self.__self, key, 0, list(self.TYPES).index(value.__class__), value.value)
        else:
            winreg.SetValueEx(self.__self, key, 0, winreg.QueryValueEx(self.__self, key)[1], value)

    def __delitem__(self, key):
        'Delete the specified value.'
        winreg.DeleteValue(self.__self, key)
    ...
    def __iter__(self):
        ...
        try:
            while True:
                values.append(winreg.EnumValue(self.__self, index)[0])
                index += 1
        except EnvironmentError:
            for value in values:
                yield value
    ...
    def __contains__(self, item):
        ...
        try:
            while True:
                if winreg.EnumValue(self.__self, index)[0].lower() == item:
                    return True
                index += 1
        except EnvironmentError:
            return False
