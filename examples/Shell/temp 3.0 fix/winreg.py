...
class Registry(object):
    ...
    def __iter__(self):
        ...
        return (Key(key, computer=self.__computer) for key in map(HKEY.__dict__.__getitem__, list(filter(str.isupper, dir(HKEY)))))
...
class Keys(object):
    ...
    def __iter__(self):
        ...
        return iter(tuple(_winreg.EnumKey(self.__key, index) for index in range(_winreg.QueryInfoKey(self.__key)[0])))

    def __contains__(self, item):
        'Check for a key\'s existence.'
        item = item.lower()
        for index in range(_winreg.QueryInfoKey(self.__key)[0]):
...
class Values(object):
    ...
    def __iter__(self):
        ...
        return iter(tuple(_winreg.EnumValue(self.__key, index)[0] for index in range(_winreg.QueryInfoKey(self.__key)[1])))

    def __contains__(self, item):
        'Check for a value\'s existence.'
        item = item.lower()
        for index in range(_winreg.QueryInfoKey(self.__key)[1]):
