...
class WabolTalk(Frame):
    ...
    FILE = 'textarea.sav'
    ERR1 = _('ERROR: There is not enough public text to cover the private text.')
    ERR2 = _('ERROR: This duplicated message may not be longer than the primer.')
    ...
    def build_widgets(self):
        self.public_frame = Labelframe(self, text=_('Public Message:'))
        self.public_text = ScrolledText(self.public_frame, **self.TEXT)
        self.private_frame = Labelframe(self, text=_('Private Message:'))
        ...
        self.decode_button = Button(self.button_frame, command=self.decode,
                                    text='\u02c4 {} \u02c4'.format(_('Decode')))
        self.wabol_frame = Labelframe(self, text=_('Wabol Message:'))
...
class OptionsBase(Dialog):
    ...
    def __init__(self, parent, min_bits, file_path, loading):
        ...
        super().__init__(parent, _('Options'))
    ...
    def build_widgets(self, master):
        self.parameter_frame = Labelframe(master, text=_('Parameters:'))
        self.primer_var = StringVar(master)
        self.primer_label = Label(self.parameter_frame, text=_('Primer Length:'))
        ...
        self.use_primer_button = Radiobutton(self.hint_pick_space,
                                             text=_('Change Primer'),
                                             command=self.handle_hint_pick,
                                             value=self.PRIMER,
                                             variable=self.hint_pick_var)
        self.use_keyprimer_button = Radiobutton(self.hint_pick_space,
                                                text=_('Alter Key/Primer'),
                                                command=self.handle_hint_pick,
                                                value=self.KEYPRIMER,
                                                variable=self.hint_pick_var)
        
        self.use_order_frame = Labelframe(master, text=_('Priority:'))
    ...
    def error(self, name):
        ...
        if quality < 0:
            tkinter.messagebox.showerror(
                _('Illegal Character'),
                _('Hint {} contains unacceptable text.').format(name.upper()),
                master=self)
            return True
        elif quality < self.min_bits:
            tkinter.messagebox.showerror(
                _('Insecure Password'),
                _('Hint {} must have greater security.').format(name.upper()),
                master=self)
            return True
        return False
...
class Options(OptionsBase):
    ...
    def body(self, master):
        # Make Language Area
        self.language_frame = Labelframe(master, text=_('Language:'))
        self.language_space = Frame(self.language_frame)
        self.language_var = StringVar(master)
        self.english_button = Radiobutton(self.language_space,
                                          text=_('English'),
                                          value='en',
                                          variable=self.language_var)
        self.spanish_button = Radiobutton(self.language_space,
                                          text=_('Spanish'),
                                          value='es',
                                          variable=self.language_var)
        # Grid New Widgets
        self.language_frame.grid(sticky=EW, **self.GRID)
        self.language_frame.grid_columnconfigure(0, weight=1)
        self.language_space.grid()
        self.english_button.grid(sticky=W, **self.GRID)
        self.spanish_button.grid(sticky=W, **self.GRID)
        # Create Parent Widgets
        super().body(master)
        self.original_language = self.language_var.get()

    def make_data_table(self):
        used, keys = super().make_data_table()
        self.options = Namespace(
            primer=Parameter(16, lambda value: value in range(16, 257)),
            hint_a=Parameter(''),
            hint_b=Parameter(''),
            key=Namespace(
                ...
                hint_pick=Parameter(self.KEY, lambda value: value in keys),
                use_order=Parameter(self.KEY, lambda value: value in keys)),
            lang=Parameter('en', lambda value: value in set(self.LANG)))
        self.data_table += ((self.language_var, 'lang'),)

    def apply(self):
        super().apply()
        current_language = self.language_var.get()
        restart = current_language != self.original_language or \
                  builtins._.__self__ is ENGLISH and \
                  current_language != 'en'
        self.result = self.result, restart
        self.LANG[current_language].install()
