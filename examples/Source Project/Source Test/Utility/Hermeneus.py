# <source name="χερμενεύς 1.1.py" size="1bfcf" hash="8171dd927465c64ab8996403fcec724c2d32e126cb4793a1a31b408e05840d5e8768917a8dfd3a81fcec0311b0521870d48e80a2c8037777165079f871193d65" />
...
##        sentinel = Blinx(root, '..')
##        sentinel.start()
        HermeneusCommands.main(root, public_path, private_path)
##        sentinel.join()
...
#9ACD32 YellowGreen'''.split('\n'))
    ...
    def __repr__(self):
        ...
        return '{}({}, {}, {})'.format(type(self).__name__, *self.__rgb)
    ...
    def invert(self):
        ...
        return type(self)(0xFF - self.r, 0xFF - self.g, 0xFF - self.b)

    def rotate(self, value):
        "Returns new color with RGB values rotated by value."
        return type(self)(self.r + value & 0xFF,
                              self.g + value & 0xFF,
                              self.b + value & 0xFF)
    ...
    def mix(self, bias, other):
        ...
        assert 0 <= bias <= 1
        alpha = 1 - bias
        return type(self)(round(self.r * alpha + other.r * bias),
...
*Release date: 24-APR-11*

- Future Plans have been reorganized slightly. List of key mappings is
  now included with a list of thoughts on functionality. Suggestions
  are encouraged to follow with examples to justify recommendations.

- File types to be supported on Source have also been listed, and help
  is requested from those that may have a special interest in a
  particular language. Documentation of comment syntax and extension
  of the source files is requested so a signature may be implemented.

- The custom import implementation in this program has been upgraded
  to be thread safe and return real modules instead of sudo modules.
  As a result, the code for automatic documentation generation was
  improved and should yield better results in the future.
...
* Add support for other languages to be allowed on Source. There are
  many different candidates available: Haskell, COBOL, Ruby, FORTRAN,
  ALGOL, Assembly, Ada, mySQL, TCL, Pascal, Forth, D, JSP, Matlab, INI
  files, Objective-C, R, Scheme, Smalltalk, TeX, VHDL, YAML, and JSON.

* Update the API of the companion program "sign.py" so that it refers
  to signing and unsigning (in and out) files instead of checking them
  (in and out). Also, the micro version number should be dropped.

* Allow word searches in the text viewer used to show documentation.

* Provide a keymapping such that the function keys work as follows:
    ESC - Close the program (done)
    F1  - Bring up a about box (done)
    F2  - Open the user's settings (done)
    F3  - Search messages logs and archive
    F4  - Close all but the main window
    F5  - Show error log for {0}
    F6  - Display window for wispering
    F7  - N/A
    F8  - Minimize or show evaluators (boss key)
    F9  - N/A
    F10 - Highlight upper menu bar (to be added)
    F11 - Toggle fullscreen/windowed display
    F12 - Export/import messages to a database
...
An interpreter who bridges the boundaries with strangers.''')
        ...
        hermeneus = import_(sys.argv[0])
        SC_REFERENCE = pydoc.plaintext.document(hermeneus)
        self.sc_reference = \
...
class Settings:
    ...
    USR_DAT = {'normal_message': Color.LightSteelBlue,
               ...
               'command_foreground': Color.FireBrick}
...
# The next function provides a better import mechanism than the previous class.

def import_(path):
    "Imports file designated by path and returns new module object."
    ...
    try:
        ...
        with open(path, errors='ignore') as file:
            source = file.read()
        filename = base.encode('ascii', 'replace')
        code = compile(source, filename, 'exec', optimize=0)
...
# The following four functions form the basis for evaluating math expressions.

def evaluate(source, local):
    "Executes all math operations found in the source."
    for expression in expressions(source):
        local['_'] = tokens(expression).evaluate(local)

def expressions(source):
    "Separates expressions and yields each individually."
...
def tokens(string):
    "Builds an expression tree by tokenizing expression."
...
# These next five classes are used in building the executable expression trees.
...
class Expression:
    ...
    def evaluate(self, bindings):
        "Calculates the value of this object."
        raise NotImplementedError()
...
class Operation(Expression):
    ...
    """Operation(left, symbol, right) -> Operation
    ...
    def __operate(self, bindings):
        "Executes operation defined by symbol."
    ...
    def split(cls, expression):
        "Splits expression on rightmost symbol."
    ...
    def __split(cls, expression):
        "Private class method: helps with split."
...
##            sys.exit()
    ...
    def configure_widgets(self):
        ...
        self.__text.tag_configure('static_link',
                                  foreground=SETTINGS.link_foreground,
                                  underline=SETTINGS.link_underline)
        # Create a tag for command codes in the text box.
        self.__text.tag_configure('command',
                                  foreground=SETTINGS.command_foreground)
    ...
    def display(self, message):
        ...
        if SETTINGS.show_timestamp:
            ...
            self.append_text(time.strftime('%I:%M %p'), 'time')
            self.append_text(' ')
        # Show the name with the proper color (message.tag).
        self.append_text('[' + message.name + ']', message.tag)
        self.append_text(' ')
    ...
    def handle_message(self, message):
        "Finds out if message is a command and takes appropriate action."
        text = message.text
        if text.startswith(':'):
            cmd, arg = (re.split('\s+', text[1:].strip(), 1) + [''])[:2]
            self.run_public_command(cmd.lower(), arg)
        else:
            self.add_text_with_URLs(text)

    def run_public_command(self, command, argument):
        "Raises an exception that a child class should override."
        raise NotImplementedError('Hermeneus cannot directly run commands.')
    ...
    def create_url(self, url):
        ...
        self.append_text(url, 'dynamic_link', tag)
    ...
    def append_text(self, string, *tag):
        "Adds message to bottom of text widget with the given tags."
        if not tag:
            self.__text.insert(tkinter.END, string)
        elif len(tag) == 1:
            self.__text.insert(tkinter.END, string, tag[0])
        else:
            self.__text.insert(tkinter.END, string, tag)

################################################################################

# This class separates the command interface from the rest of Hermeneus.

class HermeneusCommands(Hermeneus):

    "HermeneusCommands(master, public_path, **kw) -> HermeneusCommands"

    def run_public_command(self, command, argument):
        "Takes a command and tries to run it with the given argument."
        if hasattr(type(self), 'do_' + command):
            getattr(self, 'do_' + command)(argument)
        else:
            self.append_text('(error)', 'command')
            self.append_text(' Command {!r} does not exist.'.format(command))
