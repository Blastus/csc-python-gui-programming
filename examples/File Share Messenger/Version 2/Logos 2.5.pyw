...
class AboutLogos(tkinter.Toplevel):

    NEW_FEATURES = '''\
What's New in Logos 2.5?
=========================
...
- Errors will be recorded to the "Logos Settings" folder if any occur
...
- Links are automatically created to files referenced in relative to
  the program's root folder. If Logos is running on Windows, clicking
  on those links will open the file. See General Help for more info.


What's New in Logos 2.4?
=========================

*Release date: 20-Mar-11*

- Pressing F1 now brings up an "About Logos" box that allows access to
...
What's New in Logos 2.0?
=========================

* Release date: ??-Feb-11*

- Entire program was written from scratch. Logos 1 has been canceled
...
If Logos detects a special attribute of the text as described in the
...
F1 - Opens "About Logos" and displays a menu to open various bodies of
     ...
     and find a list of things yet to be accomplished in Logos.

F2 - Brings up a list of options that can be set to change the
     operation of Logos. Colors can be set by clicking on the buttons
...
Display Settings - Normally, only the past day's worth of messages are
     shown when the Logos opens. You can change this in the options to
    ...
    FUTURE_PLANS = '''\
There are no future plans for Logos at this time.'''
...
def main():
    ...
    public_path = os.path.join('CHAT LOGS', 'V2')
    private_path = os.path.join('..', 'Logos Settings')
    # Execute the main class (static) function of Logos.
    with capture_stderr() as stderr:
        Logos.main(public_path, private_path)
...
class Logos(tkinter.ttk.Frame):
    ...
    def main(cls, log_path, settings_path):
        ...
        # Create, position, and setup Logos widget for resizing.
        ...
        root.bind_all('<F1>', lambda event: AboutLogos.open_window(root))
    ...
    def get_wisper(self, message):
        ...
        # next time you implement this system for version 3 of Logos.
